...
void AppStateInPowerUpIni()
{
	...
	// 3.GOOSE开入信号初始化
	// 3.1GOOSE帧
	GoInFrame.PowerUpIni("GOOSE订阅帧", "GOOSESUB", 0, 255);
	// 3.2GOOSE控制块
	GoInCB[0].PowerUpIni("GOOSE订阅控制块0", "GOCBPUB0", GoInFrame, 255);
	GoInCB[1].PowerUpIni("GOOSE订阅控制块1", "GOCBPUB1", GoInFrame, 255);
	GoInCB[2].PowerUpIni("GOOSE订阅控制块2", "GOCBPUB2", GoInFrame, 255);
	GoInCB[3].PowerUpIni("GOOSE订阅控制块3", "GOCBPUB3", GoInFrame, 255);
	GoInCB[4].PowerUpIni("GOOSE订阅控制块4", "GOCBPUB4", GoInFrame, 255);
	GoInCB[5].PowerUpIni("GOOSE订阅控制块5", "GOCBPUB5", GoInFrame, 255);
	GoInCB[6].PowerUpIni("GOOSE订阅控制块6", "GOCBPUB6", GoInFrame, 255);
	GoInCB[7].PowerUpIni("GOOSE订阅控制块7", "GOCBPUB7", GoInFrame, 255);
	GoInCB[8].PowerUpIni("GOOSE订阅控制块8", "GOCBPUB8", GoInFrame, 255);
	GoInCB[9].PowerUpIni("GOOSE订阅控制块9", "GOCBPUB9", GoInFrame, 255);
	GoInCB[10].PowerUpIni("GOOSE订阅控制块10", "GOCBPUB10", GoInFrame, 255);
	GoInCB[11].PowerUpIni("GOOSE订阅控制块11", "GOCBPUB11", GoInFrame, 255);
	// 3.3GOOSE单点信号
...
void AppStateInGoRef()
{
	...
	{
		// FPGA提供的GOOSE接收时刻的时标，供时间同步管理使用，此处暂时不使用，时间同步管理使用本地时标更准确
		// liuzhili 修改 2022-1-11
...
// 负值超过0.5%倍量程，数据丢掉不处理，32767*0.005=163
