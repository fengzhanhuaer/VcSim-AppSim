...
//	register BYTE    bSHJ, bSTJ, bKKJ,bTJF;
	...
	// KKJ\SHJ\STJ\TJF位置判断
//	bKKJ=(pgtagIES_IMJA->bKKJEna)?g_tagIO.bDIHldIn[EN_DI_KKJ]:g_tagIO.bDIHldIn[EN_DI_OPB1_KKJ];
//	bSHJ=(pgtagIES_IMJA->bSHJEna)?g_tagIO.bDIHldIn[EN_DI_SHJ]:g_tagIO.bDIHldIn[EN_DI_OPB1_SHJ];
//	bSTJ=(pgtagIES_IMJA->bSTJEna)?g_tagIO.bDIHldIn[EN_DI_STJ]:g_tagIO.bDIHldIn[EN_DI_OPB1_STJ];
//	bTJF=(pgtagIES_IMJA->bTJFEna)?g_tagIO.bDIHldIn[EN_DI_TJF]:g_tagIO.bDIHldIn[EN_DI_OPB1_TJF];
	
	// KKJ\SHJ\STJ\TJF位置保存
	g_bFlag[EN_FLAG_DI_KKJ]=(pgtagIES_IMJA->bKKJEna)?g_tagIO.bDIHldIn[EN_DI_KKJ]:g_tagIO.bDIHldIn[EN_DI_OPB1_KKJ];
	g_bFlag[EN_FLAG_DI_SHJ]=(pgtagIES_IMJA->bSHJEna)?g_tagIO.bDIHldIn[EN_DI_SHJ]:g_tagIO.bDIHldIn[EN_DI_OPB1_SHJ];
	g_bFlag[EN_FLAG_DI_STJ]=(pgtagIES_IMJA->bSTJEna)?g_tagIO.bDIHldIn[EN_DI_STJ]:g_tagIO.bDIHldIn[EN_DI_OPB1_STJ];
	g_bFlag[EN_FLAG_DI_TJF]=(pgtagIES_IMJA->bTJFEna)?g_tagIO.bDIHldIn[EN_DI_TJF]:g_tagIO.bDIHldIn[EN_DI_OPB1_TJF];
	// KKJ计算模块,原智慧站采执规范逻辑图,目前删除。
#if 0	
	bYHJ=g_tagIO.bGoInValid[EN_GOIN_CKHZ];
	bYTJ=g_tagIO.bGoInValid[EN_GOIN_CKTZ];
	// 控合闸命令
	...
	//信号输出
	g_bFlag[EN_FLAG_DI_STJ]=pgtagIES_IMJA->bKKJ;
#endif
}
...
void IES_BusMuQH(tagIES_IMJA  *pgtagIES_IMJA)
{
	...
	{
		g_bFlag[EN_FLAG_LNK_ENA]=TRUE;
		if(g_iInter[EN_INTER_SVSUB_1M_2LINK]||g_iInter[EN_INTER_SVSUB_2M_2LINK])
		{
			g_tAlmState.bAlmIn[EN_ALM_BS_LINKQH]=FALSE;
		}
		else
		{
			g_tAlmState.bAlmIn[EN_ALM_BS_LINKQH]=TRUE;
		}
	}
	else
	{
		g_bFlag[EN_FLAG_LNK_ENA]=FALSE;
		g_tAlmState.bAlmIn[EN_ALM_BS_LINKQH]=FALSE;
	}
	...
	g_bFlag[EN_FLAG_SV_A]=bBusMUAOK;
	g_bFlag[EN_FLAG_SV_B]=bBusMUBOK;
	// 级联切换退出,是否考虑重启装置
	if((!g_bFlag[EN_FLAG_LNK_ENA])||g_tAlmState.bAlmIn[EN_ALM_BS_LINKQH])
	{
		// 默认取级联A套(如果当前B套,则切换至A套???,还是保持不变)
		if(g_bFlag[EN_FLAG_LNK_B]||(!g_bFlag[EN_FLAG_LNK_A]))
		{
			g_bFlag[EN_FLAG_LNK_B] =FALSE;
			g_bFlag[EN_FLAG_LNK_A] =TRUE;
			g_bFlag[EN_FLAG_LNK_BC]=FALSE;
			// 级联切换使能标志
			pgtagIES_IMJA->bBusMuQh=TRUE;
		}
		// 清除告警事项
		g_tAlmState.bAlmIn[EN_ALM_XZBS]=FALSE;
		return;
	}
	//刀闸位置异常合成
	bBusMUA   =g_tagIO.bDIHldIn[EN_DI_LNK_A];
	bBusMUB   =g_tagIO.bDIHldIn[EN_DI_LNK_B];
	bBusMUAuto=g_tagIO.bDIHldIn[EN_DI_LNK_AUTO];
	//选择把手状态异常
	...
	if(!pgtagIES_IMJA->dwPWInit)
	{
		...
		{
			g_bFlag[EN_FLAG_LNK_A]=FALSE;
			g_bFlag[EN_FLAG_LNK_B]=TRUE;
		}
		pgtagIES_IMJA->bBusMuQh=TRUE;
	}
	if(bBusMU==CN_MULink_BC)
	...
	g_tAlmState.bAlmIn[EN_ALM_XZBS]=bBsERR;
}
...
void IES_VolQH(tagIES_IMJA  *pgtagIES_IMJA)
{
	...
	{
		g_bFlag[EN_FLAG_V_ENA]=TRUE;
		if(g_iInter[EN_INTER_SVSUB_2M_1LINKA]||g_iInter[EN_INTER_SVSUB_2M_1LINKB]||g_iInter[EN_INTER_SVSUB_2M_2LINK])
		{
			g_tAlmState.bAlmIn[EN_ALM_BS_VQH]=FALSE;
		}
		else
		{
			g_tAlmState.bAlmIn[EN_ALM_BS_VQH]=TRUE;
		}
	}
	else
	{
		g_bFlag[EN_FLAG_V_ENA]=FALSE;
		g_tAlmState.bAlmIn[EN_ALM_BS_VQH]=FALSE;
	}

	//刀闸位置异常合成
	bXSWI1=g_tagIO.byDpiIn[EN_DPI_XSWI01];
	bXSWI2=g_tagIO.byDpiIn[EN_DPI_XSWI02];
	//电压切换投入且双母线
	if(g_bFlag[EN_FLAG_V_ENA]&&(!g_tAlmState.bAlmIn[EN_ALM_BS_VQH]))
	{
		//刀闸位置异常
	...
	if(!pgtagIES_IMJA->dwPWInit)
	{
		if(b12QH==CN_VolQH_1)
		{
			g_bFlag[EN_FLAG_V_I]=TRUE;
			g_bFlag[EN_FLAG_V_II]=FALSE;
		}
		else if(b12QH==CN_VolQH_2)
		{
			g_bFlag[EN_FLAG_V_I]=FALSE;
			g_bFlag[EN_FLAG_V_II]=TRUE;
		}
		// 上电保持状态
		else if(b12QH==CN_VolQH_BC)
		{
			g_bFlag[EN_FLAG_V_BC_INIT]=TRUE;
			g_bFlag[EN_FLAG_V_BC]=TRUE;
			g_bFlag[EN_FLAG_V_I]=FALSE;
			g_bFlag[EN_FLAG_V_II]=FALSE;
		}
	...
	g_tAlmState.bAlmIn[EN_ALM_XSWI]=bXSWIERR;
	g_tAlmState.bAlmIn[EN_ALM_PTRET]=bPtRetERR;
	g_tAlmState.bAlmIn[EN_ALM_PTACT]=bPtActERR;
	// 非保持状态,清除品质无效标志
	if(b12QH!=CN_VolQH_BC)
	{
		g_bFlag[EN_FLAG_V_BC_INIT]=FALSE;
		g_bFlag[EN_FLAG_V_BC]=FALSE;
	}
	else
	{
		g_bFlag[EN_FLAG_V_BC]=TRUE;
	...
	if(b12QH==CN_VolQH_1)
	{
		if(!g_bFlag[EN_FLAG_V_I])
		{
			g_bFlag[EN_FLAG_V_I]    =TRUE;
			g_bFlag[EN_FLAG_V_II]   =FALSE;
			g_bFlag[EN_FLAG_V_NULL] =FALSE;
			// 电压切换使能标志
			pgtagIES_IMJA->bVolQh=TRUE;
		}
	}
...
void IES_VolPub(tagIES_IMJA  *pgtagIES_IMJA)
{
	...
	if(pgtagIES_IMJA->bVolType)
	{
		if(pgtagIES_IMJA->bVolTypeRef)
		{
			IES_IMSam_SvPub_Chg(EN_ANA_SVA1_STR,EN_ANA_SAM_STR,EN_LINK_END,CN_SV_OPT_CHG3_S,CN_SV_OPT_CHG3_F);
			pgtagIES_IMJA->bVolTypeRef=FALSE;
		}
	}
	else if(g_iInter[EN_INTER_SVPUB_SUB])
	{
		if(pgtagIES_IMJA->bVolTypeRef)
		{
			dwCfg1|=CN_SV_OPT_CHG4_S;
			dwCfg2|=CN_SV_OPT_CHG4_F;
			pgtagIES_IMJA->bVolTypeRef=FALSE;
		}
#else
...
{
	BOOL bSxbyz=FALSE;
	g_tAlmState.bAlmIn[EN_ALM_OPBP1]=!g_tagIO.bDIHldIn[EN_DI_OPB1_POW];
	g_tAlmState.bAlmIn[EN_ALM_TEST]=g_tAlmState.bAlmIn[EN_ALM_SVSUB_TEST01]   \
									|g_tAlmState.bAlmIn[EN_ALM_SVSUB_TEST02]  \
									|g_tAlmState.bAlmIn[EN_ALM_GOOSE_TEST];
	// 备用遥信暂定88个,
	if(pgtagIES_IMJA->dwSxbyz)
	{
		...
		g_tAlmState.bAlmIn[EN_ALM_SXBYZ]=bSxbyz;
	}
	else
	{
		g_tAlmState.bAlmIn[EN_ALM_SXBYZ]=FALSE;
	}
}
...
void IES_IMJA_KHDX(tagIES_IMJA  *pgtagIES_IMJA)
{
	...
	bTWJ   =g_tagIO.bDIHldIn[EN_DI_OPB1_TWJ];
	bHWJ   =g_tagIO.bDIHldIn[EN_DI_OPB1_HWJ];
	bKHDX1 =!(bTWJ^bHWJ);
	// 暂时不考虑双线圈
	//g_tAlmState.bAlmIn[EN_ALM_KHDX1]=bKHDX1;
	//if(pgtagIES_IMJA->bDOpbEna)
	//{
	//bTWJ   =g_tagIO.bDIHldIn[EN_DI_OPB2_TWJ];
	//bHWJ   =g_tagIO.bDIHldIn[EN_DI_OPB2_HWJ];
	//bKHDX2 =!(bTWJ^bHWJ);
	//g_tAlmState.bAlmIn[EN_ALM_KHDX2]=bKHDX2;
	//}
	g_tAlmState.bAlmIn[EN_ALM_KHDX]=(bKHDX1|bKHDX2);
}
// ============================================================================
// 函数功能：事故总
// 输入参数：
...
void IES_IMJA_SGZ(tagIES_IMJA  *pgtagIES_IMJA)
{
	...
	g_tAlmState.bAlmIn[EN_ALM_SGZ]=g_tagIO.bDIHldIn[EN_DI_XCBR_OPN]&g_bFlag[EN_FLAG_DI_KKJ];
}
// ============================================================================
...
void IES_IMJA_BSCH(tagIES_IMJA  *pgtagIES_IMJA)
{
	...
	// 遥合(手合)
	bYHJ=g_tagIO.bGoInValid[EN_GOIN_CKHZ];
	bSHJ=g_bFlag[EN_FLAG_DI_SHJ];
	// 遥跳(手跳)
	bYTJ=g_tagIO.bGoInValid[EN_GOIN_CKTZ];
	bSTJ=g_bFlag[EN_FLAG_DI_STJ];
	// 遥跳(手跳)闭重保持标志
	...
	// 另一套闭重开入
	bBSCH=g_tagIO.bDIHldIn[EN_DI_BC_OTH];
	// GOOSE闭重发布
	bBSCHGO=bBSCHDO|bBSCH|bBSCHInit;
	// 母线CPU板闭锁重合闸信号需要由CPU间隔板提供标志
	g_tAlmState.bAlmIn[EN_ALM_BSCH]=bBSCHGO;
}
// ============================================================================
...
void IES_IMLogic_Para_Init()
{
	...
	if(g_tChkState.bChkIn[EN_CHK_PARA_FUN])
	{
		return;
	}
	pgtagIES_IMJA->dwInit=TRUE;
	//pgtagIES_IMJA->bDOpbEna=g_tagPara.dwPara[EN_PARA_ENA_DOPB];
	// 位置参数
	pgtagIES_IMJA->bDevEna =g_tagPara.dwPara[EN_PARA_TYPE_DEV];
	pgtagIES_IMJA->bKKJEna =g_tagPara.dwPara[EN_PARA_TYPE_KKJ];
	pgtagIES_IMJA->bSHJEna =g_tagPara.dwPara[EN_PARA_TYPE_SHJ];
	pgtagIES_IMJA->bSTJEna =g_tagPara.dwPara[EN_PARA_TYPE_STJ];
	pgtagIES_IMJA->bTJFEna =g_tagPara.dwPara[EN_PARA_TYPE_TJF];
	//dwTimeActSet=g_tagPara.dwPara[EN_PARA_T_KKJ];
	//IES_TimeRelaySet(&(pgtagIES_IMJA->tTimer_KKJ),T(dwTimeActSet), CN_T1S);
	// 级联切换及电压切换相关
	pgtagIES_IMJA->bBusMuQhEna =g_tagPara.dwPara[EN_PARA_ENA_LNK];
	pgtagIES_IMJA->bVolQhEna   =g_tagPara.dwPara[EN_PARA_ENA_VQH];
	// PT同时返回确认延时
	dwTimeActSet=g_tagPara.dwPara[EN_PARA_T_VQH_RET_RLY];
	IES_TimeRelaySet(&(pgtagIES_IMJA->tTimer_PTRet), T(dwTimeActSet), 0);
	// 级联A SV正常定时器
	dwTimeActSet=g_tagPara.dwPara[EN_PARA_T_SVOK];
	IES_TimeRelaySet(&(pgtagIES_IMJA->tTimer_SvA), T(dwTimeActSet), 0);
	// 级联B SV正常定时器
	IES_TimeRelaySet(&(pgtagIES_IMJA->tTimer_SvB), T(dwTimeActSet), 0);
#if(CN_SOFT_CPU_TEST_GET(CN_TEST_SAMLOCL))
	// 电压接入方式选择
	if(pgtagIES_IMJA->bVolType!=(BOOL)g_tagPara.dwPara[EN_PARA_TYPE_MSAM])
	{
		pgtagIES_IMJA->bVolType    =g_tagPara.dwPara[EN_PARA_TYPE_MSAM];
		pgtagIES_IMJA->bVolTypeRef =TRUE;
	}
#endif
	if(pgtagIES_IMJA->dwUxPhase !=g_tagPara.dwPara[EN_PARA_TYPE_UxSAM])
	{
		pgtagIES_IMJA->dwUxPhase=g_tagPara.dwPara[EN_PARA_TYPE_UxSAM];
		pgtagIES_IMJA->bUxPhaseRef=TRUE;
	}
	// 三相不一致遥信定义
	pgtagIES_IMJA->dwSxbyz     = g_tagPara.dwPara[EN_PARA_SXBYZ_YX];           // 三相不一致
	// 闭锁重合闸相关参数
	pgtagIES_IMJA->bYTBschBcEna = g_tagPara.dwPara[EN_PARA_ENA_BSBC];         // 遥分(STJ)闭锁重合闸保持投退
	dwTimeRetSet = g_tagPara.dwPara[EN_PARA_T_BSCH_INIT];                     // 装置上电闭锁重合闸返回时间
