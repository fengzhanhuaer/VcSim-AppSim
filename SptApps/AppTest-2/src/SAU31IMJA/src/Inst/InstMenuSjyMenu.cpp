...
bool8 ViewSjRecord(ApiMenu* Menu, SalSjRecordGroup* SjRecordGroup)
{
	...
	if(res==-1)
	{
		HmiSjRecordViewPool.GetValid(SjRecordGroup, 0x0, (uint64)0xffffffffffffffff);
	}
	else
	{
		dateStart = dig.Stamp();
...
bool8 ViewUserAdmSjRecord(ApiMenu* Menu)
{
	ViewSjRecord(Menu, (SalSjRecordGroup*)&SjUsrAdmRecord);
	return 0;
}
bool8 ViewControlSjRecord(ApiMenu* Menu)
{
	ViewSjRecord(Menu, (SalSjRecordGroup*)&SjContrlRecord);
	return 0;
}
...
bool8 ExportUserAdmSjRecord(ApiMenu* Menu)
{
	ExportSjRecord(Menu, (SalSjRecordGroup*)&SjUsrAdmRecord);
	return 0;
}
bool8 ExportControlSjRecord(ApiMenu* Menu)
{
	ExportSjRecord(Menu, (SalSjRecordGroup*)&SjContrlRecord);
	return 0;
}
