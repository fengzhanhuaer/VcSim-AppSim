...
namespace spt
{
	...
	public:
		...
		SalString& FormatDate(uint64 Data);
		//定点数转浮点显示，DotNum小数位数
		...
		SalString& FormatFloat(float32 Data, uint8 MinPrecision);
		SalString& FormatFixDot(float32 Data, uint8 DotNum);
	...
	typedef StaticString<65> String65B;
	...
	public:
		...
		int64 GetInt64(uint8 StopFlag);
		uint64 GetUInt64(uint8 StopFlag);
		float32 GetFloat32(uint8 StopFlag);
		float64 GetFloat64(uint8 StopFlag);
