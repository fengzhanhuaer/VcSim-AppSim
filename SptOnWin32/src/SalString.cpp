...
SalString& spt::SalString::FormatDate(uint64 Data)
{
	SalRtc t;
	t.FromStamp(Data);
	t.ToStrFmt3(*this);
...
SalString& spt::SalString::FormatFloat(float32 Data, uint8 MinPrecision)
{
	if (Data < 0)
	{
		*this << '-';
	...
	{
		*this << '.';
		for (uint32 i = 0; i < MinPrecision; i++)
		{
			Data = Data * 10;
			uint8 d = (uint8)Data;
			*this << Numer2Assic[d % 10];
			Data -= d;
		}
	}
	return *this;
...
SalString& spt::SalString::FormatFixDot(float32 Data, uint8 DotNum)
{
	int64 data = (int64)(Data * MinUint32DecData(DotNum + 1));
	Format(data, DotNum);
	return *this;
}
...
int64 spt::TransString::GetInt64(uint8 StopFlag)
{
	uint8 d;
	int64 data = 0;
	bool8 negflag = 0;
...
uint64 spt::TransString::GetUInt64(uint8 StopFlag)
{
	uint8 d;
	uint64 data = 0;
...
float32 spt::TransString::GetFloat32(uint8 StopFlag)
{
	return (float32)GetFloat64(StopFlag);
}

float64 spt::TransString::GetFloat64(uint8 StopFlag)
{
	float64 data = 0;
	uint8 d;
	bool8 negflag = 0;
